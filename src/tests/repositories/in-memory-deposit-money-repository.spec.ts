...
const makeSut = (): sutTypes => {
    ...
    const sut = new InMemoryDepositRepository()
    const createDeposit = new DepositMoneyUseCase(sut)
...
describe("test deposit-repository ", () => {
    ...
    test("the indexing should be following a sequence", async () => {
        ...
        if(user){
            ...
            expect(deposit1.id).toBe("1")
            expect(deposit2.id).toBe("2")
        }else {
